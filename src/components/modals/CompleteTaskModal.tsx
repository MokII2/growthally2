...
export default function CompleteTaskModal({ isOpen, onClose, onSubmit, taskDescription }: CompleteTaskModalProps) {
  ...
  const { toast } = useToast(); // Kept for potential future use, though not used in this simplified version

  const { control, handleSubmit, formState: { errors }, reset } = useForm<CompleteTaskFormValues>({
    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && handleCloseDialog()}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Great job! Add any notes about completing your task.
          </DialogDescription>
        </DialogHeader>
