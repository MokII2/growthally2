...
import { CheckCircle2, Star, Trophy, Clock } from "lucide-react"; // Removed Edit3
...
export default function ChildDashboardPage() {
  ...
  const handleClaimReward = async (reward: Reward) => {
    ...
    // 客户端预检查，可以提供即时反馈并禁用按钮
    if (currentPoints < reward.pointsCost) {
      toast({ title: "Cannot Claim Reward", description: "Not enough points." });
      return;
    }

    // 使用 batch 来确保两个更新操作的原子性（要么都成功，要么都失败）
    const batch = writeBatch(db);

    try {
      // --- 操作一：更新孩子自己的主文档 ---
      const userProfileRef = doc(db, "users", user.uid);
      // 使用 increment() 在服务器端进行安全的原子扣减，避免状态同步问题
      ...
      if (userProfile.parentId) {
        // 直接构建文档路径，不再需要进行无权限的查询
        // 这要求 parent/children/ 里的文档ID就是孩子的UID
        const childSubDocRef = doc(db, "users", userProfile.parentId, "children", user.uid);
        
        // 同样使用 increment() 来保持一致和安全
