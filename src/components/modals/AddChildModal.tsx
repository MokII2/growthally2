...
export default function AddChildModal({ isOpen, onClose, onChildAdded }: AddChildModalProps) {
  ...
  const processSubmit: SubmitHandler<ChildFormValues> = async (data) => {
    ...
    if (result.userProfile && result.generatedPassword) {
      toast({
      ...
      console.error("Error in AddChildModal processSubmit from signUpChildAndLinkToParent:", result.error);
    } else {
    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && handleCloseDialog()}>
      <DialogContent className="sm:max-w-lg">
        ...
        <form onSubmit={handleSubmit(processSubmit)} className="grid gap-4 py-4">
         <ScrollArea className="max-h-[calc(80vh-150px)] overflow-y-auto pr-3"> {/* Adjusted max-h and added pr for scrollbar */}
          <div className="grid gap-4"> {/* Added a wrapper div for spacing inside scroll area */}
          ...
          <DialogFooter className="mt-2 pt-4 border-t"> {/* Added border-t for separation */}
